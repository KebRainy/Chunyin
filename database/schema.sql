...
-- 图片管理相关表
-- ====================================
CREATE database IF NOT EXISTS beverage_platform;
...
CREATE TABLE IF NOT EXISTS user_behavior (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    user_id BIGINT NOT NULL,
    target_type ENUM('POST', 'BEVERAGE', 'WIKI', 'BAR') NOT NULL,
    target_id BIGINT NOT NULL,
    behavior_type ENUM('VIEW', 'LIKE', 'FAVORITE', 'COMMENT', 'SHARE') NOT NULL,
...
CREATE TABLE IF NOT EXISTS post_tag (
    ...
    tag_category ENUM('BEVERAGE_TYPE', 'TASTE', 'SCENE', 'LOCATION', 'OTHER') NOT NULL DEFAULT 'OTHER',
    source ENUM('USER', 'AUTO') NOT NULL DEFAULT 'AUTO' COMMENT '标签来源：用户输入或自动提取',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (post_id) REFERENCES share_post(id) ON DELETE CASCADE,
    UNIQUE KEY uk_post_tag (post_id, tag_name),
    INDEX idx_post_tag_post (post_id),
    INDEX idx_post_tag_name (tag_name),
    INDEX idx_post_tag_category (tag_category)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='动态标签表';
...
CREATE TABLE IF NOT EXISTS user_preference (
    ...
    tag_name VARCHAR(50) NOT NULL,
    tag_category ENUM('BEVERAGE_TYPE', 'TASTE', 'SCENE', 'LOCATION', 'OTHER') NOT NULL DEFAULT 'OTHER',
    weight DECIMAL(10,4) NOT NULL DEFAULT 0.0 COMMENT '偏好权重，根据行为累计',
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES user(id) ON DELETE CASCADE,
    UNIQUE KEY uk_user_preference (user_id, tag_name),
    INDEX idx_user_preference_user (user_id),
    INDEX idx_user_preference_tag (tag_name),
    INDEX idx_user_preference_weight (weight DESC)
